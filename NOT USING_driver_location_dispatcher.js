...
//WE USED TO USE THIS ON WEB SIMULATION DRIVER1 2 3 .PHP BUT NOW WE DONT NEED IT. (JUL 5) - IF NO ERROR, THEN YOU CAN DELETE THIS.
...
function UpdateLocations(drivers_array){
	...
	for(i=0;i<keys.length;i++){
		...
		if(i == (keys.length-1) ){
			...
			driver_guids += keys[i];
			lattitudes += drivers_array[keys[i]][0];
			longtitudes += drivers_array[keys[i]][1];
		} else {
			driver_guids += keys[i]+',';
			lattitudes += drivers_array[keys[i]][0]+',';
			longtitudes += drivers_array[keys[i]][1]+',';
	...
	general_token = createToken('1fd773046dedd607397d0509e7');

	//console.log(drivers_array[driver_guid][2]);
	var xhttp = new XMLHttpRequest();
	xhttp.onreadystatechange = function() {
	if (this.readyState == 4 && this.status == 200) {
		//Başarılı
		var responseText = xhttp.responseText.replace(/[!@#$^&%*"()+=[\]\{}|<>?,\\-]/g,'').replace(/^\s*[\r\n]/gm, '');

		var date = new Date();
		if(responseText == true ){
			console.log("\x1b[32m%s\x1b[0m","**********************************************");
			console.log("\x1b[32m%s\x1b[0m",'LOCATIONS  UPDATED '+date);
			console.log("\x1b[32m%s\x1b[0m","**********************************************");
		}else {
			console.log('LOCATIONS COULD NOT BEEN UPDATED ');
		}
	...
	xhttp.open("GET", "https://atlantisfreshdelivery.com/api/location/api-update-location.php?token="+general_token+"&&driver_guids="+driver_guids+"&&lattitudes="+lattitudes+"&&longtitudes="+longtitudes+"", true);
	xhttp.send();
...
var server_driver_location_dispatcher = require('https').createServer({
...
server_driver_location_dispatcher.listen(3000);
    ...
    socket.on('new-driver',function(data){
		...
		if(driver_names[data.DriverGUID] != data.DriverName){
			...
			// console.log("\x1b[33m%s\x1b[0m",driver_reverse);
			 ...
			 console.log("\x1b[33m%s\x1b[0m",'DRIVER ALREADY EXIST. KICKED OUT.');
			socket.disconnect();
		}
